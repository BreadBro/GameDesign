...
def score(p1name,P1score):
    ...
    dskaslg = open('scorerecords.txt', 'a')
    with open('scorerecords.txt') as s:
        records = open('scorerecords.txt','a')
...
def leaderboard():
    gamewords = open('scorerecords.txt','r')
    scores = []
...
def words():
    gamewords = open('words.txt', 'w')
    gamewords.write("python\njava\njavascript\nphp\ncomputer\ngeeks\nkeyboard\nlaptop\nheadphones\nhardware\nsoftware\nmsi\nnvidia\nlogitech\nintel\namd\nplaystation\nxbox\nnintendo\ngamecube\nryzen\ncpu\nunity")
    gamewords.close()
    gamewords = open('words.txt','r')
    words = []
    for line in gamewords:
